  ...
  return (
    <div className="market-card">
      ...
      <div className="card-value mono">
        {data ? data.value.toLocaleString('vi-VN', { maximumFractionDigits: 2 }) : '—'}
      </div>
      {data && (
...
function SignalBadge({ signal }: { signal: 'buy' | 'watch' | 'avoid' }) {
  const map = {
    buy:   { label: '▲ MUA',     cls: 'signal-buy' },
    watch: { label: '○ THEO DÕI', cls: 'signal-watch' },
    avoid: { label: '▼ TRÁNH',   cls: 'signal-avoid' },
  }
...
export default function DashboardPage() {
  ...
  // ============================================================
  // SCAN FUNCTION — Gọi 2 API routes song song
  // ============================================================
  const startScan = useCallback(async () => {
    ...
    try {
      ...
      // 2. Quét toàn bộ cổ phiếu (route này mất 5-15 giây)
      setLoadingStep(`Đang quét cổ phiếu... (có thể mất 10-15 giây)`)
      const stocksRes = await fetch('/api/stocks')

      if (!stocksRes.ok) {
        throw new Error(`Server lỗi: HTTP ${stocksRes.status}`)
      }

      const stocksJson = await stocksRes.json()
      if (!stocksJson.success) {
        throw new Error(stocksJson.error ?? 'API trả về lỗi')
      }

      setStocks(stocksJson.data.stocks)
      setSummary(stocksJson.data.summary)
    ...
    <>
      <style>{`
        ...
        .signal {
          ...
          font-size: 11px; font-family: var(--font-mono); font-weight: 700; letter-spacing: 0.5px;
        }
      ...
      <header style={{ borderBottom: '1px solid var(--border)', padding: '20px 0', position: 'sticky', top: 0, background: 'rgba(10,12,15,0.9)', backdropFilter: 'blur(12px)', zIndex: 100 }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: 'var(--muted)', fontFamily: 'var(--font-mono)', background: 'var(--surface)', border: '1px solid var(--border)', padding: '6px 14px', borderRadius: '100px' }}>
            <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: lastUpdated ? 'var(--accent)' : 'var(--muted)', animation: lastUpdated ? 'pulse 2s infinite' : 'none' }} />
            {lastUpdated ? `CẬP NHẬT ${lastUpdated}` : 'CHƯA QUÉT'}
          </div>
        </div>
      </header>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 1 }}>
        ...
        <section style={{ padding: '48px 0 32px', display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'end', gap: '32px', borderBottom: '1px solid var(--border)', marginBottom: '32px' }}>
          <div>
            ...
            <p style={{ marginTop: '12px', color: 'var(--muted)', fontSize: '15px', maxWidth: '480px', lineHeight: 1.6 }}>
              Lọc cổ phiếu theo thanh khoản, biến động, và điểm số kỹ thuật — dữ liệu thật từ TCBS.
            </p>
          </div>
          <button className="scan-btn" disabled={loading} onClick={startScan}>
            {loading ? '⏳ ĐANG QUÉT...' : stocks.length > 0 ? '↺ QUÉT LẠI' : '▶ BẮT ĐẦU QUÉT'}
          </button>
        </section>

        {/* Error */}
        {error && (
          <div style={{ background: 'rgba(255,77,109,0.1)', border: '1px solid rgba(255,77,109,0.3)', borderRadius: '10px', padding: '16px 20px', marginBottom: '24px', color: 'var(--down)', fontFamily: 'var(--font-mono)', fontSize: '13px' }}>
            ✗ LỖI: {error}
          </div>
        )}
        ...
        {stocks.length > 0 && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <div className="mono" style={{ fontSize: '12px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                Cổ phiếu đáng chú ý
              </div>
              <div style={{ background: 'rgba(0,229,160,0.1)', border: '1px solid rgba(0,229,160,0.3)', color: 'var(--accent)', fontFamily: 'var(--font-mono)', fontSize: '11px', padding: '3px 10px', borderRadius: '100px' }}>
                {displayed.length} mã
              </div>
            </div>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', background: 'rgba(0,0,0,0.2)' }}>
                    {['Mã / Công ty', 'Giá (VNĐ)', '% Thay đổi', 'KL (triệu)', 'Biến động', 'Điểm', 'Tín hiệu'].map((h, i) => (
                      <th key={h} style={{ textAlign: i === 0 ? 'left' : 'right', padding: '12px 16px', fontSize: '11px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', fontFamily: 'var(--font-mono)', fontWeight: 400, ...(i === 5 ? { textAlign: 'left' } : {}), ...(i === 6 ? { textAlign: 'left' } : {}) }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                    ...
                    return (
                      <tr key={s.ticker} style={{ borderBottom: '1px solid var(--border)', animationDelay: `${i * 25}ms`, cursor: 'pointer' }}>
                        <td style={{ padding: '14px 16px' }}>
                          ...
                          <div style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '2px' }}>{s.companyName}</div>
                        </td>
                        <td style={{ padding: '14px 16px', textAlign: 'right', fontFamily: 'var(--font-mono)', fontSize: '14px' }}>
                          {s.price > 0 ? s.price.toLocaleString('vi-VN') : '—'}
                        </td>
                        <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                          <span className={`mono ${isUp ? 'text-up' : 'text-down'}`} style={{ fontSize: '13px' }}>
        ...
        {stocks.length === 0 && !loading && (
          <div style={{ padding: '80px 24px', textAlign: 'center', color: 'var(--muted)' }}>
            ...
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '14px', lineHeight: 2 }}>
              Nhấn <strong style={{ color: 'var(--accent)' }}>BẮT ĐẦU QUÉT</strong> để lấy dữ liệu thật từ TCBS API<br/>
              Không cần đăng ký, không cần API key — miễn phí hoàn toàn
            </p>
          </div>
        )}
      </main>
      ...
      <footer style={{ borderTop: '1px solid var(--border)', padding: '24px', maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <p className="mono" style={{ fontSize: '11px', color: 'var(--muted)' }}>Data: TCBS API</p>
      </footer>
    </>
  )
}
