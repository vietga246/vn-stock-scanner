    ...
    <div className="space-y-3">
      {latestR && (
        <div className="grid grid-cols-3 gap-2">
          ...
          ] as { label: string; val: string; note: string; color?: string }[]).map(({ label, val, note, color }) => (
            ...
            <table className="w-full text-[10px]">
              ...
              <tbody>
                {cf6.map((r, i) => (
                  <tr key={i} style={{ borderTop: '1px solid #1e2832' }}>
                    <td className="p-2 font-mono" style={{ color: '#8b99a8' }}>Q{r.quarter}/{r.year}</td>
                    {([r.cfo, r.cfi, r.cff, r.capex] as (number | null | undefined)[]).map((v, j) => (
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        ] as { label: string; val: string; color: string; note: string }[]).map(({ label, val, color, note }) => (
